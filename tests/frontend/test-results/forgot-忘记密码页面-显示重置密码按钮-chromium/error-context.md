...
- Name: forgot.spec.ts >> 忘记密码页面 >> 显示重置密码按钮
- Location: forgot.spec.ts:27:7
...
Locator: locator('button:has-text("重置"), button:has-text("确认")')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button:has-text("重置"), button:has-text("确认")')
