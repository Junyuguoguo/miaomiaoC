...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long> {
     ...
     * 查询用户最近的单聊对象列表
     */
    @Query("SELECT CASE WHEN m.senderId = :userId THEN m.receiverId ELSE m.senderId END as contactId " +
           "FROM ChatMessage m WHERE m.roomId IS NULL AND (m.senderId = :userId OR m.receiverId = :userId) " +
           "GROUP BY contactId ORDER BY MAX(m.createTime) DESC")
     ...
     * 统计某个发送者发给自己的未读消息数
     */
    @Query("SELECT COUNT(m) FROM ChatMessage m WHERE m.receiverId = :userId AND m.senderId = :senderId AND m.isRead = false")
    long countUnreadBySender(Long userId, Long senderId);
}
